...
const SOURCE_HELP = {
  live: "Reads your courses on lms.unb.ca with the session signed in on this browser.",
  demo: "Shows made-up courses from the demo. Nothing is read from Brightspace.",
};

const DEBUG_LINK = "View diagnostic report";
  ...
  return `
    <section class="set-section" aria-labelledby="set-src">
      ...
      <div class="seg" role="radiogroup" aria-labelledby="set-src">
        ${opt("live", "Brightspace")}${opt("demo", "Demo data")}
      </div>
...
function template(s, courses, context) {
  ...
  const courseList = courses.length
    ? `<ul class="course-toggles">${courses
        ...
        .join("")}</ul>`
    : `<p class="set-help">Your courses show up here after UNB Now reads Brightspace.</p>`;
  ...
  <div class="settings">
    ...
    <section class="set-section" aria-labelledby="set-data">
      ...
      <p class="note" style="margin-top:6px">${icon("lock", 18)}<span>UNB Now reads Brightspace with the session that's already signed in on this browser, so it never sees your password. Your deadlines and settings stay on this computer.</span></p>
    ...
    switch (t.dataset.act) {
      case "ask-delete":
        area.innerHTML = `<div class="confirm" role="group" aria-label="Confirm delete">
          <p>This deletes everything UNB Now saved on this computer, including the items you checked off.</p>
      ...
      case "confirm-delete":
        ...
        area.innerHTML = `<p class="status-text" role="status">Your data is deleted. UNB Now has nothing saved on this computer now.</p>`;
