...
// Rules: reminders only go out for open items, stop once Brightspace shows a
...
export const LEADS = [
  ...
  { id: "morning", label: "Morning of (8 a.m.)", short: "morning of" },
];
...
export function reminderCopy(item, course, now = new Date()) {
  ...
  return { title, message: `${lead}. Still open in your saved planner.` };
}

/** Notification text for a due date that changed on Brightspace. */
