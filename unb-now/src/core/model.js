...
export function verdict(items, courses, now) {
  ...
  let line1;
  let sleepy = false;
  if (!open.length) line1 = "Everything on Brightspace is handed in.";
...
export function rowView(item, course, now) {
  ...
  // Some profs set a date the work unlocks. Until then Brightspace won't let you in,
  ...
  return {
    ...
    opensNote,
    time: fmtTime(due),
    dueState: ms < 0 ? "overdue" : sameDay(due, now) ? "today" : "future",
