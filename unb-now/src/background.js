// Service worker: reads Brightspace (the DEMO catalog or real Brightspace in LIVE mode),
...
import { trustedMessage, lmsUrl } from "./data/unb.js";
...
async function setup() {
  ...
  await ensureCatalog();
  await mutate((s) => { s.syncing = false; });
  await refreshBadge();
 ...
 * starts): if the last good read of Brightspace is more than 30 minutes old, check
...
/** Runs one GET through an open Brightspace tab's content script (learn-bridge.js). */
...
/**
 * Reads every current course from Brightspace. A course that fails is left out of the
 * fresh items and listed in `failed`, so its stored items are kept.
 */
async function readLive(source, epoch, { onSession, onCourses, onProgress } = {}) {
  ...
  if (epoch !== modeEpoch) throw abortError();
  const finalSession = await source.checkSession();
  if (!finalSession.signedIn || finalSession.student.id !== session.student.id) throw Object.assign(new Error("Brightspace account changed during collection"), { code: "signed-out" });
  const kept = assignColors(courses);
  ...
  return {
    ...
    partial: kept.some((c) => failed.has(c.id) || (source.readOk.get(c.id)?.size ?? 0) < ALL_SOURCES.length),
  };
}

/**
 * Compares a fresh read with what was stored. Keeps check-offs and marks
 * changed due dates as moved. A date that changed only because Brightspace now gives
 * it from a different field (DueDate removed, so the end date is used) is
 * updated without counting as a move. An item that is no longer on Brightspace, or
...
async function runLiveScan(settings, epoch) {
  ...
  let carry = carryOf(before);
  let firstRead = !carry.lastSyncAt && !carry.items.length;
  await mutate((s) => ({
    ...emptyState(),
    sent: s.sent || {},
    carry,
  ...
  try {
    const result = await readLive(source, epoch, {
      onSession: async (session) => {
        if (carry.student?.id === session.student.id) return;
        carry = { items: [], courses: [], student: session.student, lastSyncAt: null };
        firstRead = true;
        await mutate((s) => { s.carry = carry; s.sent = {}; s.items = []; s.courses = []; s.student = session.student; });
    ...
    const next = await mutate((s) => {
      s.termKey = source.report.term.key;
...
/**
 * A check that could not reach Brightspace. The list and every reminder stay, and
 * the next 30 minute alarm tries again. Only when the student is looking at
 * the panel and an open Brightspace tab confirms the login expired does the panel
...
/** The 30 minute check, the refresh button, and retries after Brightspace was unreachable. */
async function runLiveSync({ fromPanel = false } = {}) {
  ...
  try {
    ...
    const result = await readLive(source, epoch, { onSession: async (session) => {
      if (s0.student?.id === session.student.id) return;
      accountChanged = true;
      await mutate((s) => { s.items = []; s.courses = []; s.sent = {}; s.lastSyncAt = null; s.student = session.student; });
      await clearNotifications();
    } });
    if (epoch !== modeEpoch) return;
    if (!result.session.signedIn) {
    ...
    let moved = [];
    const next = await mutate((s) => {
      const merged = mergeLive(accountChanged ? s.items : carryOf(s).items, result.items, { failed: result.failed, readOk: result.readOk, courseIds: ids });
      moved = merged.moved;
      s.termKey = source.report.term.key;
      s.partial = result.partial;
      s.courses = result.courses;
      ...
      delete s.carry;
      if (accountChanged) skipPastReminders(s, settings);
      s.syncing = false;
      s.error = null;
      s.errorKind = null;
      s.stale = null;
...
/**
 * Retries a check that failed because Brightspace was unreachable. `wentOffline`
 * limits it to failures that happened while Chrome reported no network, so the
 * 5 minute tick only asks Brightspace again once the network is back.
 */
...
async function refreshBadge() {
  ...
  await chrome.action.setTitle({ title: n ? `${n} due on Brightspace by Sunday` : chrome.i18n.getMessage("actionTitle") });
}
...
chrome.notifications.onClicked.addListener(async (id) => {
  ...
  // Acting on a reminder is using UNB Now, even if the panel never opens.
...
async function openItem(itemId) {
  ...
  if (!item) return;
  const settings = await getSettings();
  // Brightspace answers "Not Authorized" for an item that has not opened yet, so send
  ...
  const url = lmsUrl((locked && item.listUrl) || item.url);
  if (!url) return;
  const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true });
  const isLearn = tab && lmsUrl(tab.url);
...
async function scheduleReminders() {
  ...
  const s = await getState();
  // A saved list keeps its reminders even while Brightspace can't be read.
...
/**
 * Asks Brightspace whether a dropbox folder or quiz was handed in since the last
 * read. Resolves to null when Brightspace can't be asked in time.
 */
...
async function sendDueRemindersNow() {
  ...
  const session = await source.checkSession();
  if (epoch !== modeEpoch) return;
  if (!session.signedIn || session.student.id !== snapshot.student?.id) {
    await chrome.alarms.create("reminder", { delayInMinutes: 5 });
    return;
  }
  const now = new Date();
  let due = [];
  // Claim the reminders first, so a second alarm can't send them again while
  // Brightspace is being asked.
  ...
  for (const p of due) {
    ...
    // The reminder says "you haven't submitted", so ask Brightspace first. If Brightspace
...
async function demoMove() {
  ...
  // 1. The prof changes the date on Brightspace.
  const cat = await getCatalog();
...
async function demoOnly(fn) {
  const settings = await getSettings();
  if (settings.mode === "live") return { ok: false, reason: "Demo controls only work while UNB Now shows demo data." };
  return fn();
}
...
async function handle(msg, sender) {
  if (!trustedMessage(msg, sender, chrome.runtime)) return { ok: false, reason: "Untrusted sender" };
  switch (msg && msg.type) {
    case "panel:opened": {
      await ensureCatalog();
      const s = await getState();
      if (s.needsTermRefresh || s.scan.status === "idle" || s.scan.status === "error") runScan();
    ...
    case "live:learn-page": {
      // A Brightspace page loaded. If the panel is showing the signed-out state, try again.
      const settings = await getSettings();
