/*
  ...
  @property {string} id           Stable id inside UNB Now ("cs341", or the org unit id in LIVE mode)
  ...
  @typedef {"dropbox"|"quiz"|"discussion"|"content"} LearnKind      The Brightspace tool the item lives in
  ...
  @property {string} url           The item's own page on Brightspace
  @property {string} [listUrl]     LIVE only: the course's list page for the tool, used when the item's own page would error
  @property {string} [opensAt]     ISO timestamp the item unlocks, when Brightspace gives one
  @property {"open"|"submitted"|"done"} status   submitted = Brightspace shows it; done = the student checked it off by hand
  @property {string|null} completedAt
  @property {string} [details]     Instructions text, used by the mock Brightspace pages
 ...
 * @param {() => Promise<object>} getCatalog  DEMO only: returns the fake Brightspace catalog
 * @param {object} [liveOpts]  LIVE only: { relay } for reading through an open Brightspace tab
