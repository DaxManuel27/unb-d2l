import { currentTerm, applyCurrentTerm } from "../src/data/term.js";
...
const APP = chrome.i18n.getMessage("appName") || "UNB Now";
...
function renderBar() {
  ...
  let status = "";
  if (PREVIEW) status = "";
  else if (scan === "running") status = `<span class="bar-status">Reading Brightspace</span>`;
  else if (state.syncing) status = `<span class="bar-status">Checking Brightspace</span>`;
  const busy = scan === "running" || state.syncing;
  const refresh =
    PREVIEW || (scan !== "done" && !busy)
      ? ""
      : `<button class="icon-btn" data-act="refresh" aria-label="${busy ? "Reading Brightspace" : "Check Brightspace for changes"}" ${busy ? "disabled" : ""}>${icon("refresh", 20, busy ? "spin" : "")}</button>`;
  bar.innerHTML = `
...
function scanCopy() {
  ...
  return done
    ? { title: "Done reading Brightspace", sub: "Opening your deadlines." }
    : { title: "Reading your courses on Brightspace", sub: "UNB Now uses the Brightspace session that's already signed in on this browser." };
}
  ...
  if (!root) {
    app.innerHTML = `
      <section class="scan" aria-busy="true">
        ...
        <p class="privacy">${icon("lock", 18)}<span>UNB Now never sees your Brightspace password. What it reads stays on this computer.</span></p>
      </section>`;
...
function rowHTML(r) {
  ...
  const checkLabel =
    r.status === "submitted" ? `${name} is submitted on Brightspace` : r.status === "done" ? `${name} is checked off. Select to undo.` : `Check off ${name}`;
  ...
  const moved = r.movedFrom
    ...
    : "";
  return `
  <li class="row tone-${r.tone} hl-${r.color}${r.movedFrom ? " is-moved" : ""}" data-due-state="${r.dueState}" data-id="${esc(r.id)}" data-flip="r-${esc(r.id)}">
...
/** Shown above the list when the last check couldn't read Brightspace. */
function staleHTML(now) {
  const st = state.stale;
  if (!st || !state.lastSyncAt) return state.partial ? `<div class="stale" role="status"><p>Some Brightspace sources could not be read. This list may be incomplete. Check the diagnostic report in Settings.</p></div>` : "";
  const at = new Date(state.lastSyncAt);
  const when = sameDay(at, now) ? `at ${fmtTime(at)}` : `on ${fmtDate(at)} at ${fmtTime(at)}`;
  const lead = st.kind === "signed-out" ? "Brightspace signed you out." : "Couldn't reach Brightspace.";
  const action = st.kind === "unreachable" ? "" : `<button class="link-btn stale-btn" data-act="open-learn">Open Brightspace</button>`;
  ...
  const empty =
    m.filterCourse && m.openInFilter === 0
      ? `<div class="empty" data-flip="empty">${icon("check", 22)}<h2>Nothing to hand in for ${esc(m.filterCourse.code)}.</h2><p>${
          m.groups.some((g) => g.id === "earlier") ? "Anything you already handed in is under Handed in earlier." : "New items show up here when your prof posts them on Brightspace."
        }</p></div>`
      : "";
  ...
  app.innerHTML = `
    <div class="list-view">
      ...
      <footer class="foot" data-flip="foot">
        <p>Not affiliated with D2L or the University of New Brunswick.</p>
      </footer>
    </div>`;
}
...
function renderStateView(kind) {
  const views = {
    "signed-out": {
      mark: icon("lock", 24),
      title: "Sign in to Brightspace first",
      body: "UNB Now reads Brightspace through the session in this browser. Open Brightspace and sign in. Your deadlines show up here once a Brightspace page loads. If they don't, select Try again.",
      action: `<div class="row-actions"><button class="btn btn-primary" data-act="open-learn">Open Brightspace</button><button class="btn btn-quiet" data-act="retry">Try again</button></div>`,
    },
    offline: {
      mark: icon("alert", 24),
      title: "Couldn't reach Brightspace",
    ...
    error: {
      mark: icon("alert", 24),
      late: true,
      title: "Couldn't read Brightspace",
      body: "Brightspace didn't respond. Check that you're still signed in, then try again.",
      action: `<button class="btn btn-primary" data-act="retry">Try again</button>`,
    },
    "no-courses": {
      mark: icon("info", 24),
      title: "No current-term courses found",
      body: "UNB Now couldn't confirm any enrollments for the current semester. If Brightspace lists your courses, select Try again.",
      action: `<button class="btn btn-primary" data-act="retry">Try again</button>`,
    },
    empty: {
      mark: icon("check", 24),
      title: "No deadlines found in this read",
      body: "No dated items were returned by the available APIs. Check Brightspace and the diagnostic report if you expected coursework here.",
      action: "",
    },
    deleted: {
      mark: icon("check", 24),
      title: "Your data is deleted",
      body: "UNB Now removed your deadlines and settings from this computer. It reads Brightspace again only when you ask it to.",
      action: `<button class="btn btn-primary" data-act="retry">Read Brightspace again</button>`,
    },
  };
  const v = views[kind];
...
app.addEventListener("click", (e) => {
  ...
  switch (t.dataset.act) {
    ...
    case "open-learn":
      chrome.tabs.create({
        url: settings && settings.mode === "live" ? `${liveBase(settings)}/d2l/home` : `${(settings && settings.learnBase) || "https://lms.unb.ca"}/d2l/home/`,
      });
      break;
  }
});
...
let displayedTerm = currentTerm().key;
setInterval(() => {
  const term = currentTerm().key;
  if (term !== displayedTerm) {
    displayedTerm = term;
    onState(applyCurrentTerm(state));
    if (!PREVIEW && !state.deletedAt) send("panel:opened");
  }
  if (view === "list" || view === "scan") renderBar();
  if (view === "list") {
    // Update colors as midnight/deadlines pass without replacing focused rows.
    const rows = buildModel(state, new Date(), filter).groups.flatMap((group) => group.rows);
    const dueStates = new Map(rows.map((row) => [row.id, row.dueState]));
    for (const row of app.querySelectorAll('.row[data-id]')) {
      const dueState = dueStates.get(row.dataset.id);
      if (dueState) row.dataset.dueState = dueState;
    }
  }
}, 30000);
...
(async function init() {
  ...
  if (!PREVIEW && (state.needsTermRefresh || state.scan.status === "idle" || state.scan.status === "running") && !state.deletedAt) send("panel:opened");
  // The last check couldn't read Brightspace: try again now that the student is looking.
