...
const OUTCOME = {
  "signed-out": "Brightspace said nobody was signed in.",
  "no-tab": "Brightspace said nobody was signed in, and no Brightspace tab was open to try instead.",
  unreachable: "UNB Now couldn't reach Brightspace. The network was down or Brightspace didn't answer.",
  "every-course-failed": "Brightspace answered, but every course failed to read.",
  error: "The read stopped with an error.",
  "course-failed": "The read stopped with an error.",
};

function debugSummary(report) {
  if (!report) return TESTER_BUILD ? "UNB Now hasn't read Brightspace yet. Open the UNB Now side panel while you're signed in to Brightspace." : "UNB Now hasn't read your real Brightspace account yet. Choose Live from Brightspace above, then open the side panel.";
...
async function render() {
  ...
  const demo = [
    ...
    },
    {
      ...
      cmd: "demo-reset",
      title: "Reset demo",
      body: "Puts every date and submission back and clears what UNB Now has read. Close the side panel first if you want to film the first open.",
      label: "Reset demo",
    },
  ];
  ...
  page.innerHTML = `
    ...
    ${TESTER_BUILD ? "" : `<div class="sheet">
      ...
      <section class="set-section" aria-labelledby="src-h">
        ...
        <div class="radio-list" role="radiogroup" aria-labelledby="src-h">
          <label class="radio"><input type="radio" name="mode" value="demo" ${settings.mode !== "live" ? "checked" : ""}><span><strong>Demo data</strong><span>Made-up courses from fixtures.js, shown on the mock Brightspace site.</span></span></label>
          <label class="radio"><input type="radio" name="mode" value="live" ${settings.mode === "live" ? "checked" : ""}><span><strong>Live from Brightspace</strong><span>Reads your courses on lms.unb.ca with the session signed in on this browser. Open the side panel after switching.</span></span></label>
        </div>
        <label class="field-label" for="base">Mock Brightspace address</label>
        <div class="field">
    ...
    <div class="sheet">
      <section class="set-section" aria-labelledby="dbg-h">
        ...
        <p class="set-help">Copy a diagnostic report to inspect locally. Review it before sharing; it may include course names and IDs.</p>
    ...
    <p class="page-foot">Based on WATnow by Eric Zou (MIT). Not affiliated with WATnow, D2L, or the University of New Brunswick.</p>`;
}
...
page.addEventListener("change", async (e) => {
  ...
  if (status) status.textContent = mode === "live" ? "Switched to Live from Brightspace. Open the side panel to read your courses." : "Switched to demo data.";
});
